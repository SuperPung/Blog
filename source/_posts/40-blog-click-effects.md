---
url: blog-click-effects
title: 博客页面点击效果
date: 2021-02-17 14:00:44
categories: [技术]
tags: [博客配置]
---
...
你在此网站的每一次点击，都伴随着特殊的动画特效。

效果由 JavaScript 实现。

在 `/themes/next/source/js/` 目录下新建文件夹 `cursor` ，用于存放四种特效的 `js` 文件：

```shell
cursor
  ├─explosion.min.js  // 爆炸效果
  ├─fireworks.js      // 烟花效果
  └─love.min.js       // 爱心效果
```
...
> **Custom File Support**
>
> PR [#868](https://github.com/theme-next/hexo-theme-next/pull/868) has adjusted the way of customizing the layout or style, canceling the custom files (such as `_custom/custom.styl`) in the original theme directory, and only retaining the way of specifying the custom file in the theme config file.
>
...
>
> As with [Data Files](https://hexo.io/docs/data-files), you can place all custom layouts or styles in a specific location (for example: `hexo/source/_data`). Add the custom file to `hexo/source/_data` and uncomment the content under the `custom_file_path` section in the theme config file.
...
在 `_config.next.yml` 文件中，开启 `custom_file_path` 中的 `bodyEnd`，并在 `/source/_data/` 目录下新建 `body-end.njk` 文件：
...
> 参考
>
> - [Hexo搭建个人博客系列：主题美化篇 | yearito's Blog](http://yearito.cn/posts/hexo-theme-beautify.html)
